      ...
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            ...
            {!isCollapsed && <span className={`font-medium text-sm whitespace-nowrap flex-1 transition-colors duration-200 ${isActive ? 'text-white' : 'text-gray-500 dark:text-gray-400 group-hover:text-[#7B2CBF] dark:group-hover:text-purple-300'}`}>{item.key ? t(item.key) : item.name}</span>}
