...
const sanitizeAddress = (value) => {
  ...
  const dotDatePattern = /^\d{2}\.\d{2}\.\d{4}(\s+\d{2}:\d{2})?/;
  const slashDatePattern = /^\d{2}\/\d{2}\/\d{4}(\s+\d{2}:\d{2})?/;
  const dateOnlyPattern = /^\d{4}-\d{2}-\d{2}$/;

  if (
    isoDatePattern.test(address) ||
    dotDatePattern.test(address) ||
    slashDatePattern.test(address) ||
    dateOnlyPattern.test(address)
  ) {
    return "";
  }
...
export default function Teacher() {
  ...
  const fetchTeachers = async () => {
    try {
      setLoading(true);
      const endpoint =
        activeTab === "active" ? "/teachers" : "/teachers/archive";
      ...
      const formatted = teachersData.map((item, idx) => ({
        ...
        address: sanitizeAddress(
          item.address || item.location || item.city || item.street || "",
        ),
  ...
  return (
    <>
            ...
            <thead>
              <tr className="text-[13px] text-gray-500 font-semibold border-b border-gray-50 dark:border-gray-700 bg-[#F9FAFB] dark:bg-gray-800/50">
                <th className="px-6 py-4 w-10">
                  <input
                    type="checkbox"
                    className="rounded border-gray-300 dark:border-gray-600 text-purple-600 focus:ring-purple-500 w-4 h-4"
                  />
                </th>
                ...
                <tr>
                  ...
                  >
                    <div className="flex items-center justify-center gap-3">
                      <svg
                        className="animate-spin w-5 h-5 text-purple-500"
                        fill="none"
                        viewBox="0 0 24 24"
                      >
                        <circle
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        !imageErrors[teacher.id] ? (
                          ...
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${teacher.bgColor}`}
                          >
                            {teacher.initial}
                          </div>
                        )}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 overflow-x-auto max-w-[180px] no-scrollbar py-1">
                        {teacher.group.map((g, i) => (
                          <span
                            key={i}
                            className="px-2.5 py-1 bg-[#F3F4F6] dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0"
                          >
                            {g}
                          </span>
                        ))}
      ...
      <AddTeacherModal
        ...
        teacherData={selectedTeacherForEdit}
      />
