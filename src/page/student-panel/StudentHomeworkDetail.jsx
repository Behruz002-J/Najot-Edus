...
export default function StudentHomeworkDetail() {
  ...
  const activeDetail = homeworkDetailsDb[activeId] || homeworkDetailsDb[1];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="flex-1 space-y-6 w-full">
          ...
          {activeVideoName && (
            <div className="space-y-4">
              ...
              {/* Video Box Split Screen */}
              <div className="relative aspect-[16/10] sm:aspect-[16/9.5] w-full rounded-2xl border border-gray-200/80 dark:border-gray-700/60 bg-white dark:bg-gray-800 shadow-md overflow-hidden flex flex-row group cursor-pointer">
        ...
        <div className="w-full lg:w-80 shrink-0 space-y-4">
          {homeworkDataList.map((hw) => {
            const isExpanded = expandedId === hw.id;
            const hwDetail = homeworkDetailsDb[hw.id] || { videoCount: 0 };
            
            return (
              <div key={hw.id} className="space-y-2 w-full">
                {/* Header Card */}
                <div
                  className={`w-full rounded-2xl transition-all duration-200 cursor-pointer flex items-center justify-between p-5 ${
                    isExpanded
                      ? "bg-[#E29543]/20 dark:bg-[#E29543]/15 border border-[#C08246]/30 dark:border-[#C08246]/20 shadow-sm"
                      : "bg-[#FAF8F5] dark:bg-gray-800/40 border border-[#E7E2D6] dark:border-gray-700/60 hover:bg-gray-50 dark:hover:bg-gray-700/50"
                  }`}
                  onClick={() => {
                    if (isExpanded) {
                      setExpandedId(null);
                    ...
                    {hwDetail.videoCount > 0 ? (
                      Array.from({ length: hwDetail.videoCount }).map((_, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className="bg-[#EFC79E] dark:bg-[#4a3522]/60 py-3 px-5 rounded-2xl flex items-center gap-3 hover:bg-[#ebbd8d] dark:hover:bg-[#4a3522]/80 transition-all duration-150 cursor-pointer shadow-sm group text-left"
                            onClick={(e) => {
                              e.stopPropagation();
                          ...
                          >
                            ...
                            <span className="text-sm font-semibold text-[#163656] dark:text-gray-200 tracking-tight">
